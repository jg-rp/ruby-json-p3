...
class TestJSONPointer < Minitest::Test
  def test_to_string
    pointer = JSONP3::Pointer.new("/some/thing/1")

    assert_equal("/some/thing/1", pointer.to_s)
  end

  def test_resolve_root
    pointer = JSONP3::Pointer.new("")
  ...
  def test_missing_key
    pointer = JSONP3::Pointer.new("/some/other")
    data = { "some" => { "thing" => "else" } }

    assert_equal(JSONP3::Pointer::UNDEFINED, pointer.resolve(data))
  end

  def test_index_out_of_range
    pointer = JSONP3::Pointer.new("/some/thing/7")
  ...
  def test_property_of_array
    pointer = JSONP3::Pointer.new("/some/thing/else")
    data = { "some" => { "thing" => [1, 2, 3] } }
  ...
  def test_no_leading_slash
    assert_raises(JSONP3::Pointer::SyntaxError) { JSONP3::Pointer.new("some/other") }
  end
  ...
  def test_trailing_slash
    pointer = JSONP3::Pointer.new("/foo/")
  ...
  def test_index_with_leading_zero
    pointer = JSONP3::Pointer.new("/some/thing/01")
    data = { "some" => { "thing" => [1, 2, 3] } }
  ...
  def test_resolve_with_parent
    pointer = JSONP3::Pointer.new("/some/thing")
    data = { "some" => { "thing" => [1, 2, 3] } }
  ...
  def test_resolve_root_with_parent
    ...
    assert_equal(JSONP3::Pointer::UNDEFINED, parent)
  ...
  def test_resolve_missing_with_parent
    ...
    assert_equal(data["some"], parent)
    assert_equal(JSONP3::Pointer::UNDEFINED, item)
  end
  ...
  def test_parent_of_pointer
    data = { "some" => { "thing" => [1, 2, 3] } }
    pointer = JSONP3::Pointer.new("/some/thing/0")
  ...
  def test_parent_is_root
    data = { "some" => { "thing" => [1, 2, 3] } }
    pointer = JSONP3::Pointer.new("/some")
    parent = pointer.parent
  ...
  def test_pointer_exists_falsy_value
    data = { "some" => { "thing" => [1, 2, 3] }, "other" => nil }
    pointer = JSONP3::Pointer.new("/other")
  ...
  def test_pointer_does_not_exist
    data = { "some" => { "thing" => [1, 2, 3] }, "other" => nil }
    pointer = JSONP3::Pointer.new("/nosuchthing")
